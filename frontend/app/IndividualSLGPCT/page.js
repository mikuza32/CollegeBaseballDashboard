...
export default function IndividualSluggingPercentageCharts() {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                ...
                console.error("Could not retrieve Slugging Percentages", error);
            }
        }
        fetchData();
    }, []);
    ...
    useEffect(() => {
        ...
        let sortedData = [...data];
        if (order === "Ascending") {
            sortedData.sort((a, b) => parseFloat(a["SLG PCT"]) - parseFloat(b["SLG PCT"]));
        } else if (order === "Descending") {
            sortedData.sort((a, b) => parseFloat(b["SLG PCT"]) - (a["SLG PCT"]));
        ...
        const dataset = limitData.map(item => {
            const num = parseFloat(item["SLG PCT"]);
        ...
        setChartData({
            ...
            datasets: [
                {
                    label: "Individual Slugging Percentage",
                    data: dataset,
    ...
    return (
        <div className="bg-black text-gray-50 p-6 rounded-lg shadow-lg">
            <h2 className="text-2xl font-semibold mb-4">Individual Slugging Percentage</h2>
            <select
            ...
            <Bar
                ...
                options={{
                    ...
                    plugins: {
                        ...
                        title: {
                            display: true,
                            text: 'Names and Slugging Percentage',
                            color: '#e0e0e0',
                        },
                    },
                    scales: {
