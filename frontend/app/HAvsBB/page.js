...
export default function HAvsBB() {
    ...
    useEffect(() => {
        ...
        const scatterPlot = data.map(item => ({
            x: parseFloat(item.HA),
            y: parseFloat(item.BB),
            Name: item.Name,
            Team: item.Team
        }));
        ...
        setChartData({
            datasets: [
                {
                    label: 'Hits Allowed vs. Walks Allowed',
                    data: scatterPlot,
    ...
    const chartOptions = {
        ...
        plugins: {
            title: {
                display: true,
                text: 'Hits Allowed vs. Walks Allowed',
                color: '#e0e0e0',
            },
            legend: {
                labels: {color: '#e0e0e0'},
        ...
        scales: {
            ...
            y: {
                title: {display: true, text: 'Walks Allowed', color: '#e0e0e0'},
                ticks: {color: '#e0e0e0'},
                grid: {color: 'rgba(255,255,255,0.1)'},
            },
        },
    ...
    return (
        <div className=" min-h-screen bg-black text-gray-50 p-6 rounded-lg shadow-lg  overflow-x-auto">
            ...
            <h1 className=" text-4xl font-extrabold text-center mb-10">NCAA Division 1 Baseball: Hits Allowed vs. Walks Allowed</h1>
            {chartData ? (
